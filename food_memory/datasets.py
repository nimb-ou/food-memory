...
def load_food101(split: str):
    ...
    try:
        ...
        raise RuntimeError('Food-101 loading requires `datasets`. Install project dependencies with `pip install -r requirements.txt`.') from exc
...
def iter_food101(split: str, samples_per_class: int | None=None, seed: int=42) -> Iterator[DatasetImage]:
    ...
    for idx, row in enumerate(source):
        ...
        counts[label_id] += 1
        yielded += 1
        yield DatasetImage(source_id=f'{split}:{idx}', split=split, label_id=label_id, label_name=label_names[label_id], image=image)
